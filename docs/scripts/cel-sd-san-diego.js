...
(function () {
  ...
  /* RESPONSIVE AUDIT round 3, 2026-09-08 — the filter above stops a dead row from
     BEHAVING like a control; it still leaves it VISIBLE. Measured on the published
     page at every width: `#offers` and `#accommodation` resolve to null, so 2 of
     the 12 rows are a labelled dot and a cursor:pointer that scroll nowhere — and
     on the mobile drawer they are 2 of 12 rows of a list the reader has to page
     through. Hiding the row is the honest render: the section is not on this page.
     Removed from the a11y tree too, so a screen reader does not announce a
     destination that does not exist.
     This is deliberately a RUNTIME hide, not a markup deletion. The two sections
     exist in the local mirror and not on the live page (recorded in the report as
     mirror drift); if either is ever built, the row reappears on its own with no
