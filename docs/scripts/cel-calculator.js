/*!
 * CEL — cel-calculator.js
 * The SHARED calculator/planner engine, verbatim from the design project's
 * shared/calculator.js. One engine, many calculators: it owns reading controls,
 * click and keyboard wiring, slider fill, number tweening, row visibility,
 * verdict panels, chips, root state classes and reduced-motion handling.
 *
 * Kept as its OWN bundle rather than folded into a page bundle so the San Diego
 * budget planner and the costs-page calculator can share one copy — inlining it
 * would ship 21 KB of identical code per page and fork on the next edit.
 *
 * LOAD ORDER: this file, then the page config (cel-calculator-<page>.min.js).
 * They communicate through window.CELCalculator — the engine defines mount() and
 * drains a __queue, so a config that loads first queues instead of failing.
 */
/* ═══════════════════════════════════════════════════════════════════════════
