...
def _isolate_summary_state(tmp_path, monkeypatch):
    ...
    # Block-level TM (tools.summary.block_reuse): isolate too, else a live-mode test
    # reuses real seeded blocks and skips Gemini (e.g. the submit_batch-count assertion).
    monkeypatch.setattr(config, "BLOCK_TM_FILE", tmp_path / "block-translation-memory.json")
