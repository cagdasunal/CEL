...
def render_translations_html() -> str:
    ...
    # Intro
    parts.append('    <section class="status status-ok">')
    parts.append('      <p class="status-label">Weglot CSV downloads — Fidelo + summary translator.</p>')
    parts.append('      <p>One ZIP bundle and one CSV per non-English locale. Each CSV contains <strong>Fidelo rows</strong> (course and housing on-page text: titles, bullets, USP facts, accordion sections) and <strong>translator rows</strong> (summary paragraphs and static page meta tags generated by the summary translator). Import each per-language CSV into the Weglot Dashboard to override machine translations with authoritative copy.</p>')
    parts.append("    </section>")
