...
def render_translations_html() -> str:
    """Render /admin/translations/ — Weglot CSV downloads (Fidelo + translator rows).
    ...
    Each per-locale CSV contains two types of rows:
    - Fidelo rows: course and housing on-page text (titles, bullets, USP facts).
    - Translator rows: summary paragraphs (type=Text) and static page meta tags
      (type=meta_title / meta_description) generated by the summary translator.
    """
    now = now_san_diego()
    parts = []
    parts.append("<!DOCTYPE html>")
    ...
    parts.append('  <meta name="description" content="Download Weglot import CSVs: Fidelo on-page text plus translator-generated summary paragraphs and page meta tags.">')
    parts.append('  <meta name="robots" content="noindex, nofollow">')
    parts.append(f'  {render_favicon_tag()}')
    parts.append('  <link rel="stylesheet" href="/assets/css/dashboard.css">')
    parts.append("</head>")
    ...
    # Intro
    parts.append('    <section class="status status-ok">')
    parts.append('      <p class="status-label">Weglot CSV downloads — one ZIP, Fidelo + Summaries kept separate.</p>')
    parts.append('      <p>Download the single ZIP below. It unzips into two folders: <strong>Fidelo/</strong> (course &amp; housing on-page text — titles, bullets, USP facts, accordion sections) and <strong>Summaries/</strong> (the summary paragraphs and page meta generated by the translator), one <code>&lt;lang&gt;.csv</code> per non-English locale in each. Import each CSV into the Weglot Dashboard (with “Overwrite existing translations” on) to replace machine translations with authoritative copy.</p>')
    parts.append("    </section>")

    parts.append("    <h2>Weglot import files</h2>")
    parts.append('    <ul class="files">')

    # The ONE download — a ZIP with Fidelo/ + Summaries/ folders.
    parts.append("    <li>")
    parts.append('      <div class="file-row">')
    parts.append('        <span class="file-name">Weglot translations — Download all (ZIP: Fidelo/ + Summaries/, all 8 languages)</span>')
    ...
    parts.append(f'      <p class="subtle">{note_zip} — unzip, then import each <code>Fidelo/&lt;lang&gt;.csv</code> and <code>Summaries/&lt;lang&gt;.csv</code> in the Weglot Dashboard.</p>')
    parts.append("    </li>")
