...
# === Localization desk (/admin/localization/) ===
# Adds NO new colour, radius or type value: every declaration resolves to a token
# already defined in SHARED_CSS :root. The button hierarchy is the one the
# change-password modal established -- one filled indigo pill for the single
# primary action, ghost pills for everything else, .5 opacity when disabled.
DESK_CSS = """
  /* Locale index cards */
  .desk-locales { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 14px; margin: 18px 0 0; }
  /* The card is a container, not a link: it holds a link AND a Discard button, and a
     button inside an anchor is invalid and unreachable by keyboard. But it must LOOK and
     ACT clickable (operator 2026-09-23: "there is nothing clickable in here"), so the
     title link is stretched over the whole card, the chips and Discard sit above that
     layer and stay separately clickable, and a button-styled label says what a click
     does. */
  .desk-locale-card {
    position: relative; cursor: pointer;
    display: block; padding: 18px; color: inherit;
    background: var(--panel); border: 1px solid var(--border);
  ...
  .desk-locale-name { display: inline-block; margin: 0; font-size: var(--fs-lg); font-weight: 600; }
  .desk-locale-sub { margin: 2px 0 12px; font-size: var(--fs-sm); color: var(--muted); }
  ...
  /* Toolbar */
  .desk-toolbar { display: flex; flex-wrap: wrap; align-items: flex-end; gap: 14px; }
  .desk-field { display: flex; flex-direction: column; gap: 4px; font-size: var(--fs-sm); color: var(--muted); }
  .desk-select {
  ...
  .desk-btn.is-on:hover { background: var(--stripe); }
  .desk-btn.is-flagged { background: rgba(166,95,31,0.15); color: var(--warn); border-color: var(--notice-border); }

  /* Rows. Dense on purpose -- this is a few hundred rows of triage, not a form. */
  /* Why this row is worth a second look. Advice, so it is quiet: one line, warm
     colour, under the text it is about. It never proposes a replacement. */
  .desk-why {
    margin: 5px 0 0; font-size: var(--fs-sm); color: var(--notice-fg);
  ...
  /* ONE signal per row: a soft background, and nothing else. No dimming (it made the
     reviewer's own finished work the hardest thing to read, and said "disabled" rather
     than "done"), and no colour on the buttons (a filled green tick next to a green row
     said the same thing twice, in a way that competed with it).

     Each tint means something, and the meanings do not overlap:
       green  -> settled, going to the website
       indigo -> the machine owes us something
       warm   -> waiting on YOU to read it
       red    -> it went wrong
  ...
  /* One line, always. Wrapping four pills onto three lines made every row look
  ...
  /* The active choice is marked by WEIGHT, not colour -- the row's background already
     says which state it is in, and a second colour saying the same thing competed with
     it. Clicking the marked one undoes it. */
  .desk-icon-btn.is-on { background: transparent; color: var(--fg); border-color: var(--fg); }
  .desk-icon-btn.is-on:hover { background: var(--stripe); }
  ...
  /* Under the edit box, only when the text carries link markers. */
  .desk-editor-note { margin: 6px 0 0; font-size: var(--fs-xs); color: var(--muted); }
  .desk-search { min-width: 210px; }
  /* Words that are a link on the website. Weglot's <a wg-N=""> markers are not shown in
     reading view; a dotted underline says "this is a link" without the markup. No
     accent colour -- that is reserved for focus and the one primary (ruling 21). */
  .desk-linktext { text-decoration: underline dotted; text-underline-offset: 3px; }
  ...
  /* Sticky action bar. Two zones: the selection you are acting on now (top, only
     ...
     either is non-empty). Both can be visible at once -- they answer different
     questions ("what happens if I click Approve" vs "what is waiting to be sent"). */
  .desk-savebar {
    position: sticky; bottom: 0; z-index: 20;
    display: flex; flex-direction: column; gap: 0;
    margin-top: 18px;
    background: rgba(241,234,216,0.94);
    -webkit-backdrop-filter: blur(8px); backdrop-filter: blur(8px);
    border: 1px solid var(--border-strong); border-radius: var(--radius);
    box-shadow: 0 -6px 24px rgba(55,51,44,0.08);
  }
  .desk-savebar[hidden] { display: none; }
  .desk-bar-row {
    display: flex; align-items: center; gap: 12px; flex-wrap: wrap;
    padding: 11px 16px;
  }
  .desk-bar-row + .desk-bar-row { border-top: 1px solid var(--border); }
  .desk-bar-row[hidden] { display: none; }
  .desk-savebar-text { margin: 0; font-size: var(--fs-md); font-variant-numeric: tabular-nums; }
  .desk-savebar-text strong { font-weight: 600; }
  .desk-savebar-spacer { flex: 1 1 auto; }

  /* Tray chips -- also used on the locale index */
  .desk-tray {
    display: inline-flex; align-items: center; gap: 7px;
  ...
  .desk-trays[hidden] { display: none; }
  .desk-status { min-height: 18px; font-size: var(--fs-md); color: var(--muted); }
  .desk-status.is-ok { color: var(--ok); }
  .desk-status.is-error { color: var(--err); }

  /* Notice used for the not-yet-wired batch step */
  .desk-notice {
    margin: 0; padding: 10px 14px; font-size: var(--fs-md);
    color: var(--notice-fg); background: var(--notice-bg);
  ...
  .desk-queue-item:last-child { border-bottom: 0; }
  .desk-queue-src {
