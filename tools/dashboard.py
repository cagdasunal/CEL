...
DESK_CSS = """
  ...
  .desk-btn.is-on { background: var(--accent); color: #fff; border-color: var(--accent); }
  ...
  /* A machine proposal sits BESIDE what it would replace, never on top of it --
     accepting without seeing what is being replaced is not review. */
  .desk-proposed {
    display: block; margin-top: 6px; padding: 7px 10px;
    background: rgba(93,96,238,0.08); border-left: 3px solid var(--accent);
    border-radius: 0 var(--radius-sm) var(--radius-sm) 0;
  }
  .desk-proposed[hidden] { display: none; }
  .desk-proposed-label {
    display: block; font-size: var(--fs-xs); font-weight: 600; letter-spacing: 0.04em;
    text-transform: uppercase; color: var(--accent); margin-bottom: 2px;
  }
  .desk-proposed-text { display: block; }
  .desk-row.is-proposed > td { background: rgba(93,96,238,0.04); }
  .badge-proposed {
    display: inline-block; border-radius: 3px; padding: 1px 6px;
    font-size: var(--fs-xs); font-weight: 600;
    background: rgba(93,96,238,0.14); color: var(--accent);
  }
