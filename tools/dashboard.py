...
DESK_CSS = """
  ...
  /* The card is a container, not a link: it holds a link AND an Undo button, and a
     button inside an anchor is invalid and unreachable by keyboard. Only the title
     is the link, so the whole card is not one giant tap target that swallows Undo. */
  .desk-locale-card {
